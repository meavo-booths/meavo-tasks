...
| Shared-board `VIEWER` | That board | View only |
| `EXTERNAL` assignee | Single task | View that task only, never edit (`isExternalOnly`) |
...
- Rules agents get wrong without docs:
  ...
  - `EXTERNAL` assignees can *view* a task without any workspace access; never grant them edit paths.
